...
- **idf_use_crt_bundle** (*Optional*, bool): Only with ``esp-idf``. If true, the [esp-idf Certificate Bundle](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/protocols/esp_crt_bundle.html) feature will be enabled, which includes a trusted source of certificates from Mozilla (containing more than 130 certificates). Keep in mind that this list should be updated often for each esp-idf release, or else the certificates could expire.
