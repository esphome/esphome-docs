...
- **swap_x_y** (*Optional*, boolean): If true the x and y axes are swapped, i.e. the raw sensor. Defaults to ``false``.
...
- **on_state** (*Optional*, :ref:`Automation <automation>`): An automation to perform
  when the touchscreen is pressed or released. See :ref:`xpt2046-on_state`.
