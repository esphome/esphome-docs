...
- **full_update_every** (*Optional*, int): E-Paper displays have two modes of switching to the next image: A partial
  ...
  all other models.
- **reset_duration** (*Optional*, int): Duration in milliseconds for the display reset operation. Defaults to ``200``.
  Setting this value to ``2`` may resolve issues with newer e-Paper Driver modules (e.g. Rev 2.1).
