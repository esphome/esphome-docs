    ...
    display:
      - platform: tm1637
        ...
        lambda: |-
          ...
          // Print the current SNTP time without leading zero
          it.printf(0,"%2d.%02d",id(sntp_time).now().hour,id(sntp_time).now().minute);
          // Result for 7:06:42 -> " 7:06" on a display with : and " 7.06" on a display with .
