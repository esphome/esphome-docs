...
Noone knows what exact interval is - it varies based on your electrical environment - some migght need it every hour, for example. 
Without this picture on LCD might get glitchy after some time.
You can plan refresh by using ``interval:`` section and calling ``request_refresh()`` function, after than it will perform display 
