...
- **model** (**Required**, string): The display model to use. One of the following options:
  ...
  - ``TTGO TDisplay 135x240`` Automatically specifies required pins.
  ...
  - ``LILYGO T-Embed 170X320`` requires ``power_supply`` be specified, but automatically specifies ``dc_pin``, ``cs_pin``,
    ``backlight_pin`` and ``reset_pin``.
  - ``Custom`` (see details below)

- **cs_pin** (*Optional*, :ref:`Pin Schema <config-pin_schema>`): The CS pin.
- **dc_pin** (**Required**, :ref:`Pin Schema <config-pin_schema>`): The DC pin - required except for some boards with presets.
- **reset_pin** (**Required**, :ref:`Pin Schema <config-pin_schema>`): The RESET pin - also has presets on some boards.
- **height** (*Optional*, int): Sets height of display in pixels.
- **width** (*Optional*, int): Sets width of display.
...
For specific models there is some preset configuration. All the above models except ``Custom`` preset the ``width``, ``height``
``offset_width`` and ``offset_height`` since these are inherent to the display itself. For models that are part of a complete
board (currently the two Lilygo boards, the Adafruit Funhouse and S2) the ``cs_pin``, ``dc_pin``, ``backlight_pin`` and ``reset_pin`` are preset.

SPI clock and data pins must be configured in an ``spi:`` component (for the T-Embed board these are 12 and 11 respectively).

A power supply component must be configured and specified for some boards (E.g. T-Embed, and Adafruit S2.)

For all boards you can override the presets by specifying any of the configuration options. Pin assignments on some
boards can change between versions.
