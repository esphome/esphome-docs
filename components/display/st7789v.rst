...
module, you'll likely want to add a ``colors:`` section to your YAML configuration; please see
...
.. code-block:: yaml
    ...
    display:
        ...
        lambda: |-
          // predefined:
          auto gray = GRAY.to_rgb_565();
          // custom:
          auto red = id(my_red).to_rgb_565();

The ``to_rgb_565()`` method puts the color data into the format required for this display.
...
- uses several predefined colors
...
.. code-block:: yaml
    ...
    display:
      - platform: st7789v
        ...
        rotation: 270
        lambda: |-
          auto red = RED.to_rgb_565();
          auto yellow = YELLOW.to_rgb_565();
          auto green = GREEN.to_rgb_565();
          auto blue = BLUE.to_rgb_565();
          auto gray = GRAY.to_rgb_565();
