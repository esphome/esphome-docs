...
By default the **measurement_time** is set to ``69`` which will result into to measurements up to 54612.5 lx for this sensor. For low-light situtations consider to choose a higer measurement_time up to ``254`` which will result in a maximum measurement range up to 14835 lx. For sunny scenes (for example outdoors with sunlight) use lower values down to ``31`` which will give you the maximum measurement range up to 121556 lx
...
- **address** (*Optional*, int): Manually specify the i^2c address of the sensor.
  ...
  the address is ``0x5C``.
- **measurement_time** (*Optional*, int): Manually specifiy the measurement time between ``31`` and ``254``. Defaults to ``69``
