...
- **max_move_distance** (*Optional*, int): Maximum distance for movement detection.
  Value between ``0.75m`` and ``6m`` inclusive. Defaults to ``6m``.
  All options from :ref:`Number <config-number>`.
...
- **distance_resolution** (*Optional*): control the distance resolution. Can be ``0.75m`` or ``0.2m``. Defaults to ``0.75m``.
  All options from :ref:`Select <config-select>`.
- **baud_rate** (*Optional*): control the baud rate. Defaults to ``256000``.
  Once changed, need to reinstall with an updated :ref:`UART Component <uart>` configuration.
  All options from :ref:`Select <config-select>`.
