...
.. note::
    ...
    The config can be changed at runtime using lambdas. The api can be found here :apiref:`sx127x/sx127x.h`. Settings, except for mode changes, will only be applied after calling configure.
...
.. note::
    ...
    Currently a workaround is needed to make this work. Remote transmitter setup must be called again after a GPIO mode change. Upgrading to a newer versions of the RMT driver will resolve this issue.
