...
- **on_packet** (*Optional*, :ref:`Automation <automation>`): An automation to perform when a packet has been decoded. A variable x of type std::vector<uint8_t> is passed to the automation for use in lambdas.
...
SX127X has a number of :ref:`actions <config-action>`

``sx127x.set_mode_tx``

This :ref:`action <config-action>` sets the SX127X mode to tx, use ``sx127x.send_packet`` in packet mode.
