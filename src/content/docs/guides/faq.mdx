...
You can use the [ESPHome Device Builder](/install/) directly; after editing your device's
...
- Too many clients simultaneously connected to the native API server on the device may also result in this behavior.
  ...
  [ESPHome Device Builder](/install/) each establish a connection to the device. In
...
Running ESPHome from the official container image is covered in
[Running ESPHome in Docker](/install/docker/): image tags, running the Device Builder, a Compose file, USB
passthrough, and the host networking that mDNS depends on.

<span id="docker-reference-notes"></span>

> [!NOTE]
> ESPHome uses mDNS to discover devices and resolve their `.local` names, and that is how the
> [ESPHome Device Builder](/install/) tracks whether a device is online. Anything that stops multicast reaching
> ESPHome breaks it: running in Docker without host networking (see
> [Running ESPHome in Docker](/install/docker/)), or a network where your server and your devices sit on different
> subnets or VLANs.
>
> Where mDNS can't reach, a periodic ICMP ping sweep still tracks every device, so one stays visible as long as its
> address resolves some other way - a literal IP, or a hostname served by regular DNS. A `.local` name may still
> work, because the Device Builder retries the bare hostname without the suffix, which your DNS may serve. Devices
> that resolve no other way are shown as offline.
>
...
>
> If your router supports Avahi, you can make mDNS work across subnets. For example, in OpenWRT or pfSense:
>
...
- Status detection in the [ESPHome Device Builder](/install/)
