...
import lvglGradientConicalImg from './images/lvgl_gradient_conical.png';
...
- **pad_row** (*Optional*, int16): Set the padding between the rows of the children elements, in pixels. See [Layouts](/components/lvgl/layouts#lvgl-layouts) for details. Some non-container widgets also use this, for example the [chart](/components/lvgl/widgets#chart) widget's `STACKED` bars.
- **pad_column** (*Optional*, int16): Set the padding between the columns of the children elements, in pixels. See [Layouts](/components/lvgl/layouts#lvgl-layouts) for details. Some non-container widgets also use this, for example the gap between a [chart](/components/lvgl/widgets#chart) widget's `BAR`/`STACKED` columns.
...
- **direction** (**Required**, string): The direction of the gradient: `hor` (or `horizontal`), `ver` (or `vertical`), `linear`, `radial` or `conical`.
- **linear** (*Optional*): Configures the gradient when `direction` is `linear`. See [below](#linear-gradients) for details. Required when `direction` is `linear`.
- **radial** (*Optional*): Configures the gradient when `direction` is `radial`. See [below](#radial-gradients) for details. Required when `direction` is `radial`.
- **conical** (*Optional*): Configures the gradient when `direction` is `conical`. See [below](#conical-gradients) for details. Required when `direction` is `conical`.
...
The `hor` and `ver` directions are a simple shorthand for a gradient that spans the whole width or height of the
object it is applied to. The `linear`, `radial` and `conical` directions give more control over where the gradient
is drawn, and support an `extend` option that controls what is drawn once the pixel being painted falls outside
the range covered by the stops:

- **PAD** (the default): The color of the nearest stop is used, so the first and last colors spread out to fill
  the remaining space.
- **REPEAT**: The sequence of stops starts over from the beginning, so the gradient tiles.
- **REFLECT**: The sequence of stops plays backward before starting over, so each tile is a mirror image of the
  one before it, avoiding a visible seam at the point where it repeats.

What counts as "outside the range" depends on the gradient type:

- For a **linear** gradient, it's the area beyond the `from` and `to` points, on either side of the line.
- For a **radial** gradient, it's the area further from the center than the `to` point - beyond the outer edge
  of the circle the stops are painted on.
- For a **conical** gradient, it only matters when `start_angle` and `end_angle` are less than a full circle
  apart; it's the remainder of the circle that the sweep doesn't cover.

Every coordinate in the `linear`, `radial` and `conical` sections accepts either a number of pixels or a
percentage of the object's size, and can be negative or greater than 100% to place a point outside the object.

#### Linear gradients

A `linear` gradient runs in a straight line between two points, `from` and `to`. Unlike the `hor`/`ver` shorthand,
the line does not need to be horizontal, vertical, or confined to the object's bounds.

- **from_x**, **from_y** (**Required**, pixels or percentage): The starting point of the gradient line.
- **to_x**, **to_y** (**Required**, pixels or percentage): The ending point of the gradient line.
- **extend** (*Optional*, string): One of `PAD`, `REPEAT` or `REFLECT`. Defaults to `PAD`.
...
A `radial` gradient radiates outward from a center point. The distance from `center` to `to` sets the radius at
which the gradient reaches its final stop. An optional focal point can be set to offset the center of the color
rings from the center of the circle, producing an off-center "spotlight" effect.

- **center_x**, **center_y** (**Required**, pixels or percentage): The center point of the gradient.
- **to_x**, **to_y** (**Required**, pixels or percentage): A point on the outer edge of the gradient; its distance from the center sets the gradient's radius.
- **focal_x**, **focal_y** (*Optional*, pixels or percentage): The center point of the color stops, if different from `center_x`/`center_y`. Both must be specified together.
- **focal_radius** (*Optional*, int): The radius of the focal point. Defaults to `0`.
- **extend** (*Optional*, string): One of `PAD`, `REPEAT` or `REFLECT`. Defaults to `PAD`.
...
A `conical` gradient sweeps around a center point, like the hands of a clock, between a start and end angle.

- **center_x**, **center_y** (**Required**, pixels or percentage): The center point the gradient sweeps around.
- **start_angle** (*Optional*, 0-360): The angle, in degrees, at which the gradient starts. Defaults to `0`.
- **end_angle** (*Optional*, 0-360): The angle, in degrees, at which the gradient ends. Defaults to `360`.
- **extend** (*Optional*, string): One of `PAD`, `REPEAT` or `REFLECT`. Defaults to `PAD`.
...
Sweeping the full 0-360° range with the same rainbow stops used in the `hor` example above produces a wheel
showing the complete hue gamut. A `radial` gradient only ever varies one property - color - along its radius, so
it cannot add "white at the center, fading to full color at the edge" to the same widget: that takes a second,
white-to-transparent `radial` gradient layered in a widget of its own on top of the hue wheel, letting the hue
show through more and more towards the edge. Setting `radius: circle` on both widgets - which only has a visible
effect because their width and height are equal - clips the square widgets to a circle, so the result looks like a
traditional color wheel rather than a hue gradient with square corners:
    ...
    - obj:
        ...
        widgets:
          - obj:
              ...
              border_width: 0
              pad_all: 0
              bg_grad: white_center
```

<Image src={lvglGradientConicalImg} layout="constrained" alt="A color wheel made from a conical hue gradient and a radial white-to-transparent overlay" />
