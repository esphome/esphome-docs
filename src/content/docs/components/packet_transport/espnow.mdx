...
The [Packet Transport Component](/components/packet_transport) platform allows ESPHome nodes to directly communicate with each other
...
> [!NOTE]
> The `provider` field must match the node name configured in the provider device’s `esphome:` block; this name is included in the `packet_transport` data.
...
```yaml
esphome:
  name: room-sensor-1  # room-sensor-2, outdoor-sensor on the other devices
