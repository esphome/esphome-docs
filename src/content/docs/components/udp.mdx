...
- **addresses** (*Optional*, list of IP addresses): One or more IPv4 or IPv6 addresses to send data to. Defaults to `255.255.255.255`
  which is the local network broadcast address. IPv6 addresses require a platform with IPv6 support (ESP32-IDF, HOST, Zephyr).

- **listen_address** (*Optional*, IPv4 or IPv6 multicast address): Joins a multicast group and listens on that address. Defaults to no
  multicast address, just the local network broadcast address `255.255.255.255`. IPv6 multicast addresses (e.g. `ff02::1`) require a
  platform with IPv6 support (ESP32-IDF, HOST, Zephyr). **NOTE**: When set, the socket binds to the multicast group address. For IPv4,
  this means broadcast packets (`255.255.255.255`) are no longer received. IPv6 has no broadcast equivalent. Outgoing `addresses` are
...
On platforms with IPv6 support (ESP32-IDF, HOST, Zephyr), both `listen_address` and `addresses` accept IPv6 multicast addresses:

```yaml
udp:
  listen_address: ff02::1
  addresses:
    - ff02::1
```
