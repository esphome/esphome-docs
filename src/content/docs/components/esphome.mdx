...
- **loop_interval** (*Optional*, [Time](/guides/configuration-types#time)): Set the loop interval of the main loop. Increasing from the default `16 ms`
  value can be useful for low power applications but increases latency. **Attention:** Some components might not work properly with increased intervals.
  Setting a smaller interval is usually not needed as components will request it them selfs. 
- **suspend_loop** (*Optional*, boolean): If set, suspends the main loop if no component needs it in order to allow longer sleep times and reduced power consumption. If
  at least one component does not disable its loop this has no effect (examples are `api` or `wifi`). **Attention:** Some components might not work properly or fail to update.
