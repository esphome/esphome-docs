...
When powering the module from the indoor unit's internal USB port, this rail is shared with the unit's own display board on some models and WiFi transmit current spikes can sag it enough to cause display flicker or dropout. A 470-1000µF, 16V (or higher) electrolytic capacitor across 5V/GND at the module's power input, placed as close to the module as possible, resolves this on affected units.
