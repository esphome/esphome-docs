...
| [Hisense/BGH (Argentina)](#hisense_arg) | `hisense_arg`                    | yes |
...
<span id="hisense_arg"></span>

### `hisense_arg`

The `hisense_arg` platform supports Hisense and BGH-branded air conditioners sold in Argentina,
including Tadiran units that respond to the BGH Smart Control (HPA-4911) WiFi module.

The protocol uses a 3-frame IR transmission (21 bytes total) with NEC-like pulse-distance encoding.
Power on/off is tracked as a state transition rather than a mode code, so the component maintains
internal power state.

> [!NOTE]
> The remote receiver requires a higher idle timeout to capture the full 3-frame signal.
> Set `idle: 50ms` and `buffer_size: 1024` on your `remote_receiver` configuration.
...
This protocol uses a power **toggle** command rather than discrete on/off codes. The component
tracks power state internally, but it can get out of sync if the AC is controlled by another
remote or powered externally. When state is wrong, the next command may accidentally toggle
power instead of changing settings.

Two methods are exposed to sync the assumed power state with an external source (such as a
power consumption sensor in Home Assistant):

- `set_power_off()`: Forces the component to assume the AC is off (sets mode to OFF).
- `set_power_on()`: Forces the component to assume the AC is on (sets mode to heat/cool as unknown).

```yaml
# Example: buttons for power state sync from Home Assistant
button:
  - platform: template
    name: "AC Sync Power Off"
    on_press:
      - lambda: id(hisense_ac).set_power_off();

  - platform: template
    name: "AC Sync Power On"
    on_press:
      - lambda: id(hisense_ac).set_power_on();
```
...
- <APIRef text="hisense_arg.h" path="hisense_arg/hisense_arg.h" />
