...
| Noblex | `noblex`                           | yes |
| [Remko, Fischer ClimaButler, TROTEC, Novamatic, Rexair, FREECOM](#remko_ar715) | `remko_ar715` | |
...
<span id="remko_ar715"></span>

### `remko_ar715`

Supports air conditioners using the **Remko AR-715** IR remote (model 1613138).

| Brand | Model |
|---|---|
| Remko | RKL 490, RKL 491, RKL 494, RKL 495 |
...
The fan speed **Auto** corresponds to the remote's *Fuzzy Logic* mode, where the
unit selects the fan speed automatically based on room temperature.

> [!NOTE]
> The IRremoteESP8266 library refers to this protocol as *ClimaButler* after one
> of the rebranded models. The OEM and original manufacturer is Remko.
>
> Fan speed selection is only available in Cool mode.
> In Dry and Fan Only mode the unit controls the fan speed automatically.
>
> The hardware timer function (1–24h) of the AR-715 remote is not implemented.
> Use Home Assistant automations for equivalent functionality.
>
> Heat and Heat/Cool modes are not supported by the AR-715 remote and are
> not available in this component.

```yaml
# Example configuration entry
climate:
  - platform: remko_ar715
...
- <APIRef text="remko_ar715.h" path="remko_ar715/remko_ar715.h" />
