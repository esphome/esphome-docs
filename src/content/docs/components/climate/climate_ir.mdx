...
| [GREE](#gree_ir) | `gree`                             | yes (`yb1fa` and `yx1ff`) |
...
The `yb1fa` and `yx1ff` models can also receive commands from the original remote and update the climate state. They
decode power, operating mode, target temperature from 16 to 30 °C, fan speed, and vertical swing on/off. The `yx1ff`
model also decodes the sleep preset. Both models only expose vertical swing control, so horizontal and combined swing
modes are not offered.

The `yb1fa` model maps the three numbered fan speeds to low, medium, and high. The `yx1ff` model maps them to quiet,
low, and medium, and uses the Turbo bit for high.

A GREE command has an inter-block space of approximately 20 ms. Set `remote_receiver.idle` to a value longer than
that space so the receiver does not split one command into two captures. Use `25ms` to `30ms`; the default `10ms` is
too short for this protocol.
...
The receiver updates only the state encoded by the remote. If `sensor` is configured, its current temperature reading
is kept unchanged.

The following additional features can be controlled with switches. Availability depends on the selected GREE model:

| Model | `light` | `turbo` | `health` | `xfan` |
| --- | --- | --- | --- | --- |
| `yan`, `yaa`, `yac`, `yac1fb9` | yes | yes | yes | yes |
| `yb1fa` | yes | yes | no | yes |
| `yx1ff` | yes | no | no | no |

On `yb1fa` and `yx1ff`, received commands also update the configured supported switches. ESPHome stores the received
feature state in the climate component and preserves it when a later climate or feature change transmits a complete
GREE state frame.

Feature switches for these receive-capable models do not restore a saved state or transmit IR during startup. They
initially publish the protocol default state and are synchronized by the first valid frame received from the physical
remote.

On `yb1fa`, Turbo is represented as an effective high fan speed. Enabling the Turbo switch also updates the climate fan
state to high. Selecting any fan mode through the climate entity disables Turbo; disabling only the Turbo switch leaves
the regular high fan speed selected. The `yx1ff` Turbo bit is part of high fan-speed encoding and is therefore not
available as an independent switch.

- **gree_id** (**Required**, [ID](/guides/configuration-types/#id)): The ID of the `gree` climate to which the switches
  belong.
- **light** (*Optional*, [Switch](/components/switch/#config-switch)): Turns the indoor-unit display/LED on or off.
- **turbo** (*Optional*, [Switch](/components/switch/#config-switch)): Enables maximum fan speed for faster results.
- **health** (*Optional*, [Switch](/components/switch/#config-switch)): Enables the air ionization feature.
- **xfan** (*Optional*, [Switch](/components/switch/#config-switch)): Keeps the indoor fan running briefly after the
  climate device is turned off to dry the evaporator.
