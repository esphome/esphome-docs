...
> [!NOTE]
> On the `zephyr` platform, WiFi is only supported on the `esp32` and `esp32c6` variants — see the
> [Zephyr platform's variant table](/components/zephyr/#variants). It is not available on `esp32h2` or
> `native_sim`.
...
- **min_rssi** (*Optional*, string): Only on `zephyr`. The minimum signal strength a scanned network must have to be
  considered as a connection candidate at all, e.g. `-85dB`. Accepts values from `-100dB` to `-20dB`. Weaker scan
  results are discarded before any connection attempt is made. If not specified, no filtering is applied (all scan
  results are considered, down to the lowest signal strength the radio reports).
