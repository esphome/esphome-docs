...
Always `true` on the `nrf52` platform because IPv4 is not supported. On the `zephyr` platform the default varies
by variant (`true` on `esp32h2` and `nrf52`, since both default to using `openthread:`, which requires IPv6;
`false` otherwise).
