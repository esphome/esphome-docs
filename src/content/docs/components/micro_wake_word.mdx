...
- **models** (*Optional*, list): The models to use. Only the first model is enabled by default on the first boot. Each model's enabled state is then saved/restored to/from the flash.
  Defaults to an empty list, which builds the firmware without any wake word models.
