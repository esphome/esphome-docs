...
This automation will be triggered when a new state (Role) occurs. The new state will be given as the variable `x`, typed as
the OpenThread `otDeviceRole` enum, and can be used in [lambdas](/automations/templates#config-lambda).

Here are the states (Roles), along with the value logged when printed with a `%d` format specifier as below
...
Since `x` is a real `otDeviceRole` value, a [lambda](/automations/templates#config-lambda) can compare it against the
named `OT_DEVICE_ROLE_*` constants instead of the raw number:
...
openthread:
  on_state:
    then:
      - lambda: |-
          if (x == OT_DEVICE_ROLE_LEADER) {
            ESP_LOGI("main", "Device is now the Leader");
          }
```
...
will be the new value, and `x_previous` is the value immediately prior to the change. Both these parameters are typed as the
OpenThread `otDeviceRole` enum.
