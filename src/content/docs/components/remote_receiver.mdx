...
- **dump** (*Optional*, list): Decode and dump these remote codes in the logs (at log.level=DEBUG).
  ...
  - **onkyori**: Decode and dump Onkyo RI codes. See
    [transmitter description](/components/remote_transmitter#remote_transmitter-transmit_onkyori)
    for wiring and voltage-level details.
...
- **onkyori**: Trigger on a decoded Onkyo RI code with the given data.
  ...
  - **data** (**Required**, int): The Onkyo RI code to trigger on, in the range `0x000` to `0xFFF`, see dumper
