...
- **dump** (*Optional*, list): Decode and dump these remote codes in the logs (at log.level=DEBUG).
  ...
  - **rc5x**: Decode and dump RC5X (Marantz Extended RC5) infrared codes.
...
- **on_rc5x** (*Optional*, [Automation](/automations)): An automation to perform when a
  RC5X (Marantz Extended RC5) remote code has been decoded. A variable `x` of type <APIStruct text="remote_base::RC5XData" path="remote_base::RC5XData" />
  is passed to the automation for use in lambdas.
...
- **rc5x**: Trigger on a decoded RC5X (Marantz Extended RC5) remote code with the given data.

  - **address** (**Required**, int): The address to trigger on, see dumper output for more info.
  - **command** (**Required**, int): The RC5X command to listen for.
  - **extension** (**Required**, int): The RC5X extension field to listen for.
