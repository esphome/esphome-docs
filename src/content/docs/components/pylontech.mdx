...
- **cells** (*Optional*): Entries to monitor individual cells. See [Monitoring individual cells](#monitoring-individual-cells).
...
The cells inside each battery can also be monitored.
...
sensor:
  - platform: pylontech
    ...
    cells:
      - cell: 0
        voltage:
          name: "Battery1 Cell0 Voltage"
        temperature:
          name: "Battery1 Cell0 Temperature"
      - cell: 1
        voltage:
          name: "Battery1 Cell1 Voltage"
        temperature:
          name: "Battery1 Cell1 Temperature"
```

- **cell** (**Required**): Which cell to monitor. Starts counting from zero.
- **voltage** (*Optional*): Voltage of the singular cell. All options from [Sensor](/components/sensor).
- **temperature** (*Optional*): Temperature of the singular cell. All options from [Sensor](/components/sensor).

### `pylontech.set_cell_polling` Action

Monitoring individual cells causes some additional overhead.
In case it is not always needed, it can be dynamically disabled.

```yaml
switch:
  - platform: template
    name: "Pylontech poll individual cells"
