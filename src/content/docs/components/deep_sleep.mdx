...
This applies to both the `nrf52` platform and every `zephyr` platform hardware variant (`native_sim` does not
support `deep_sleep` at all). When `sleep_duration` is configured, or Zigbee is used (only possible on the
`nrf52` platform — the `zephyr` platform does not support Zigbee on any variant), the device resumes normal
operation after waking up. Otherwise, it performs a full reset instead of continuing execution.
