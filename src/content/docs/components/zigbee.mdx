...
- **cluster** (*Optional*, enum): Zigbee cluster to be used. One of `basic` and `default`. `basic` uses basic analog/binary Zigbee
  ...
  names are not exposed when using `default`. On `nRF52` only basic clusters are supported. Defaults to `basic`.
- **endpoint** (*Optional*, int): Endpoint number in the range from 1 to 239. This defines which entity goes on which
  ...
  two entities of the same type, such as two sensors, cannot share an endpoint unless `cluster: default` maps them to
  different clusters (for example a temperature and a humidity sensor).If more than one entity on an endpoint
...
`cluster = default` is supported for `temperature`, `humidity`, `atmospheric_pressure`, `pressure`, `volume_flow_rate`, `illuminance`, `pm25`, `carbon_dioxide` device classes.
