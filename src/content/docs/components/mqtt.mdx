...
- **tls** (*Optional*, bool): Only on ESP32. Whether to use TLS for the MQTT connection. Defaults to `false`. Make sure to change the `port` of the MQTT broker. Most brokers use port 8883 for TLS connections.
> [!TIP]
...
- **verify_ssl** (*Optional*, bool): Only on ESP32. When set to `true` (default), SSL/TLS certificates will be validated upon connection; if invalid,
  the connection will be aborted. To accomplish this, ESP-IDF’s default ESP x509 certificate bundle is included in the build. This certificate bundle
  includes the complete list of root certificates from Mozilla’s NSS root certificate store.
- **certificate_authority** (*Optional*, string): Only on ESP32. CA certificate in PEM format. See [TLS CA (ESP32)](#mqtt-tls-ca-idf) for more information.
...
<span id="mqtt-tls-ca-idf"></span>

## TLS CA (ESP32)

On ESP32, if the server's CA certificate is required to validate the TLS connection,
