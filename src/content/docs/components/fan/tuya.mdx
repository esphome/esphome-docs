...
- **optimistic** (*Optional*, boolean): Whether to operate in optimistic mode. When in
   this mode, any command sent to the Fan will immediately update the reported
   state without waiting for MCU feedback. Defaults to `false`.
