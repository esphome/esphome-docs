---
description: "Instructions for setting up the I²C bus to communicate with 2-wire devices in ESPHome"
title: "I²C Bus"
---
import APIRef from '@components/APIRef.astro';


<span id="i2c"></span>

This component sets up the I²C bus for your `esp32`, `esp8266`, `rp2040`, `nrf52`, `zephyr`, or `host` platform. In order for these components
to work correctly, you need to define the I²C bus in your configuration. On ESP32 (both Arduino and ESP-IDF
frameworks), internal pullup resistors are enabled by default. On ESP8266, the internal 10kΩ pullup resistors
are always enabled. External pullups are still recommended for longer wire runs or multiple devices.
You can use multiple devices on one I²C bus as each device is given a
unique address for communicating between it and the ESP. You can do this by hopping
wires from the two lines (SDA and SCL) from each device board to the next device board or by
connecting the wires from each device back to the two I²C pins on the ESP.

```yaml
# Example configuration entry for ESP32
i2c:
  sda: GPIOXX
  scl: GPIOXX
  scan: true
  id: bus_a
```

```yaml
# Example configuration entry for host platform
i2c:
  device: /dev/i2c-1
  scan: true
```

## Configuration variables

- **id** (*Optional*, [ID](/guides/configuration-types#id)): Manually specify the ID for this I²C bus if you need multiple I²C buses.

- **scan** (*Optional*, boolean): If ESPHome should do a search of the I²C address space on startup.
  Defaults to `true`.

- **frequency** (*Optional*, frequency): Set the frequency the I²C bus should operate on.
  Defaults to `50kHz`. Values are `10kHz`, `50kHz`, `100kHz`, `200kHz`, ... `800kHz`.

### `esp32`, `esp8266`, `rp2040`, `nrf52`

- **sda** (*Optional*, [Pin](/guides/configuration-types#pin)): The pin for the data line of the I²C bus.
  Defaults to the default of your board (usually GPIO21 for ESP32 and GPIO4 for ESP8266).

- **scl** (*Optional*, [Pin](/guides/configuration-types#pin)): The pin for the clock line of the I²C bus.
  Defaults to the default of your board (usually GPIO22 for ESP32 and GPIO5 for ESP8266).

- **timeout** (*Optional*, [Time](/guides/configuration-types#time)): Set the I²C bus timeout.
  Defaults to the framework defaults (`100us` on `esp32`, `1s` on `esp8266` and `1s` on `rp2040`).
  Maximum on `esp32` is 13ms.

### `esp32` only

- **sda_pullup_enabled** (*Optional*, boolean): Enable the internal pullup resistor for the SDA pin.
  Defaults to `true`.

- **scl_pullup_enabled** (*Optional*, boolean): Enable the internal pullup resistor for the SCL pin.
  Defaults to `true`.

- **low_power_mode** (*Optional*, boolean): Enable the low-power (master only) I²C bus.
  Only available on ESP32C5, ESP32C6 and ESP32P4. Defaults to `false` unless required.

### `host`

- **device** (**Required**, string): The I²C device path to use (e.g., `/dev/i2c-0`, `/dev/i2c-1`).

### `zephyr`

- **sda**, **scl** (*Optional*, [Pin](/guides/configuration-types#pin)): If not set, ESPHome tries to read the
  pin assignments from the board's devicetree. If they can't be determined this way (for example, a custom
  board with flexible pin muxing, like nRF52), both must be set explicitly.
- **device** (*Optional*, string): A Linux I²C device path (e.g. `/dev/i2c-1`) to pass through to the host's
  real I²C bus. Only supported on the `native_sim` variant.
- **dts_node_override** (*Optional*, string): The devicetree node label of the I²C controller to use (e.g.
  `i2c0`), for when ESPHome can't automatically determine which of the board's I²C buses is enabled. Only
  needed if config validation reports it couldn't resolve the bus automatically.
- **emulation** (*Optional*, list): Defines one or more emulated I²C devices instead of a real bus, useful for
  testing sensor/output components without physical hardware. Supported on all `zephyr` variants. Each
  item has:

  - **id** (**Required**, [ID](/guides/configuration-types#id)): The ID to reference this emulated device by.
  - **address** (**Required**, int): The I²C address to emulate.
  - **registers** (**Required**, mapping): Register address to canned response value(s). A scalar returns the
    same byte on every read; a list cycles through single-byte values on successive reads; a list of
    same-length lists cycles through multi-byte reads.

  ```yaml
  # Example configuration entry
  zephyr:
    variant: nativesim

  i2c:
    emulation:
      - id: fake_bme280
        address: 0x76
        registers:
          0xD0: 0x60 # chip ID
          0xF7: [[0x50, 0x00, 0x00, 0x80, 0x00, 0x80, 0x00]] # pressure/temp/humidity burst read
  ```

  `emulation:` and `device:` are mutually exclusive on the same bus; `emulation:` wins if both are set.

> [!NOTE]
> Only a single I²C bus is currently supported on the `zephyr` platform.

## Multiple I²C buses

If the device supports multiple I²C buses, they need to be defined separately and sensors need to be set up specifying the correct bus:

```yaml
# Example configuration entry
i2c:
  - id: bus_a
    sda: GPIOXX
    scl: GPIOXX
    scan: true
  - id: bus_b
    sda: GPIOXX
    scl: GPIOXX
    scan: true
# Sensors should be specified as follows
sensor:
  - platform: bme680
    i2c_id: bus_b
    address: 0x76
```

For I²C multiplexing see [Tca9548A](/components/tca9548a/).

## See Also

- [TCA9548A I²C Multiplexer](/components/tca9548a/)
- [Generic I²C device component](/components/i2c_device/)
- [Host Platform](/components/host/)
- <APIRef text="i2c.h" path="i2c/i2c.h" />
