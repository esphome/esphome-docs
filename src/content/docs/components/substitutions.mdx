...
`Path` objects support the following properties:

- `.parent`: Returns a new `Path` to the parent folder
- `.name`: Returns the file or folder name (last component of the path)
- `.parts`: Returns a tuple of the path components

The `/` operator appends a path component with the appropriate folder separator.
...
For all available properties and methods, refer to the
[Python `PurePath` documentation](https://docs.python.org/3/library/pathlib.html#pure-paths).

#### `this` in included files

When using `!include`, `this` is scoped to the included file. Each included file gets its own
`this.file` and `this.dir` referring to its own location, not the location of the file that
includes it. This makes it easy to reference files relative to the included file:

```yaml
# main.yaml
esphome:
  includes:
    - !include my_component/component.yaml
```

```yaml
# my_component/component.yaml — this.dir refers to my_component/, not the parent folder
esphome:
  includes:
    - ${this.dir}/helper.h
```
