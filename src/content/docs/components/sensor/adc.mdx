...
- Zephyr: Available pins vary by variant, see [Zephyr](#zephyr) below.
...
- **attenuation** (*Optional*): On ESP32, or on the `zephyr` platform with an `esp32`/`esp32c3`/`esp32c6`/`esp32h2`
  variant, or together with `emulation:`. Specify the ADC attenuation to use. See
  [ESP32 Attenuation](#esp32-attenuation). Defaults to `0db`.
...
Pin availability on the `zephyr` platform's `esp32`/`esp32c3`/`esp32c6`/`esp32h2` variants matches ADC1 in
[ESP32 pins and Hardware Details](#adc-esp32_pins) above (ADC2 is not exposed). `attenuation:` behaves the
same as on ESP32, defaulting to `0db`.

On the `nrf52` variant, `pin:` is a GPIO number that maps to one of the SoC's SAADC analog inputs
(`AIN0`-`AIN7`); which GPIO maps to which `AIN` channel is fixed by the chip, not configurable.
`attenuation:` does not apply on this variant — gain and reference are fixed internally.
...
- **emulation** (*Optional*, list): Defines canned voltage reading(s) for this sensor instead of a real ADC
  channel — useful for testing without hardware, typically with the `native_sim` variant. `pin:` is still
  required by schema but is ignored. Each entry is one `sample()` call's worth of readings, either a single
  voltage (sugar for a one-reading group) or a list matching `samples:` for multisampling; successive updates
  cycle through the list of entries.

  ```yaml
  # Example configuration entry
  sensor:
    - platform: adc
      pin: GPIO0 # ignored, but still required
