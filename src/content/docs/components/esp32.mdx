...
- **watchdog_timeout** (*Optional*, [Time](/guides/configuration-types#time)): The timeout applied to the
  ...
  windows. Defaults to `5s`, valid values are `0s` and values in the range of `5s` to `60s`. Note: OTA updates temporarily raise the
  watchdog to `15s` when the configured timeout is below that threshold.  Values between `0s` and `5s` are ignored and `5s` is used instead.
  Using a value of `0s` turns off the ESP-IDF task watchdog functionality.
...
- **enable_full_printf** (*Optional*, boolean): Enable full FILE\*-based printf support. By default, ESPHome wraps
  ...
  On the ESP32-C6, builds using [HTTP Request](/components/http_request/) also no longer link newlib's printf engine
